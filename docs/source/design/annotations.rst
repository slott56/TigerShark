...
-   **Structural**. A :py:meth:`parse` methods all gather source text and apply the overall
...
-   **Elemental**. For Composites, a :py:meth:`build` method is used to construct
...
The :py:meth:`__init__` methods for ``Segment`` and ``Composite`` perform
...
..  sidebar:: Functional Approach

    An approach could be to have each validator return a Monad
    ...
    This is **not** the approach we've chosen.

A separate :py:mod:`x12.base.X12Parser` class
has a collection of :py:meth:`parse` methods.

For Message, Loop, Segment there are :py:meth:`parse`, :py:meth:`loop_parse`, and :py:meth:`segment_parse`
methods to consumes zero or more complete segments
from the input.
For Composite parsing within a Segment,
a :py:meth:`composite_build()` method consumes fields from the segment to
build Composites.
...
-   ``Annotated[list[Annotated[X, etc.], MaxItems()]``.
    This is often represented as a inner ``TypeAlias`` for ``Annotated[X, etc.]``.
    The overall list requires a separate maximum items validation that's not (currently) built into the class.

-   ``Union[Annotated[X, etc.], None]``.
